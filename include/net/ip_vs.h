...
/* kABI fix for commit 749c42b (ipvs: reduce sync rate with time thresholds) */
	...
	struct ip_vs_seq fnat_seq;    	/* for fullnat */
...
#ifndef __GENKSYMS__
	/* kABI fix of commit 8599928 (IPVS: Add struct ip_vs_pe)
	 * and commit e9e5eee (IPVS: Add persistence engine to connection entry)
	 */
	const struct ip_vs_pe	*pe;
	char			*pe_data;
	__u8			pe_data_len;
...
extern struct ip_vs_service *
ip_vs_lookup_vip(int af, __u16 protocol, const union nf_inet_addr *vaddr);
 ...
 *	Lookup route table
 */
extern struct rtable *ip_vs_get_rt(struct sk_buff *skb, union nf_inet_addr *addr, u32 rtos);

#ifdef CONFIG_IP_VS_IPV6
extern struct rt6_info *ip_vs_get_rt_v6(struct sk_buff *skb, union nf_inet_addr *addr);
#endif
