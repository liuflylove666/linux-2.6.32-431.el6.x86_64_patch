					     ...
					     bool inverse)
{
    ...
    return ip_vs_conn_hashkey(p->af, p->caddr, p->cport, p->vaddr, p->vport);
}




static unsigned int ip_vs_conn_hashkey_conn(const struct ip_vs_conn *cp, bool inverse)
{
	struct ip_vs_conn_param p;


	if (likely(!inverse)) {
		// false  OUTside2INside
		ip_vs_conn_fill_param(cp->af, cp->protocol, &cp->caddr, cp->cport, &cp->vaddr, cp->vport,
								&p);
	} else {
		// true   INside2OUTside
		ip_vs_conn_fill_param(cp->af, cp->protocol, &cp->daddr, cp->dport, &cp->laddr, cp->lport,
				      &p);
	}
	...
	return ip_vs_conn_hashkey_param(&p, inverse);
}
...
__ip_vs_conn_get(const struct ip_vs_conn_param *p, int *res_dir)
{
	unsigned hash;
...
struct ip_vs_conn *ip_vs_conn_get(const struct ip_vs_conn_param *p, int *res_dir)
{
	struct ip_vs_conn *cp;

	cp = __ip_vs_conn_get(p, res_dir);
	if (!cp && atomic_read(&ip_vs_conn_no_cport_cnt)) {
		struct ip_vs_conn_param cport_zero_p = *p;
		cport_zero_p.cport = 0;
		cp = __ip_vs_conn_get(&cport_zero_p, res_dir);
	}
	       ...
	       struct sk_buff *mskb, int is_synproxy_on)
{
	...
	ci_idx = (struct ip_vs_conn_idx *)(((__u8 *) cp) + sizeof(struct ip_vs_conn));
	...
	co_idx =(struct ip_vs_conn_idx *)(((__u8 *) cp) +
	...
	if (is_synproxy_on == 1 && mskb != NULL) {

		th = skb_header_pointer(mskb, ip_hdr(mskb)->ihl * 4,
					sizeof(_tcph), &_tcph);
		if (th == NULL) {
		...
		/* Save ack packet */
		skb_queue_tail(&cp->ack_skb, mskb);
		/* Save ack_seq - 1 */
...
static int ip_vs_conn_seq_show(struct seq_file *seq, void *v)
{
	...
	if (v == SEQ_START_TOKEN)
		seq_puts(seq,
   "Pro FromIP   FPrt ToIP     TPrt LocalIP  LPrt DestIP   DPrt State       Expires PEName PEData\n");
	else {
